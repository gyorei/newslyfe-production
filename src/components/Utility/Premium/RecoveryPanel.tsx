...
const RecoveryPanel: React.FC = () => {
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      setError('Hálózati hiba vagy szerverhiba.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 400, margin: '32px auto', padding: 24, border: '1px solid #eee', borderRadius: 8, fontFamily: 'sans-serif' }}>
      <h3>Restore license key with recovery code</h3>
      ...
      >
        {loading ? 'Visszaállítás...' : 'Visszaállítás'}
      </button>
      {restoredKey && (
        <div style={{ marginTop: 16, padding: 12, background: '#f0fff0', borderRadius: 4, color: 'green' }}>
          <strong>Siker!</strong> A visszaállított licenckulcs:
