...
function formatRelativeTime(dateString: string, t: (key: string, options?: any) => string): string {
  ...
  if (diff < 60) return t('videoCard.timeAgo.seconds', { count: diff });
  
  const min = Math.floor(diff / 60);
  if (min < 60) return t('videoCard.timeAgo.minutes', { count: min });
  
  const hours = Math.floor(min / 60);
  if (hours < 24) return t('videoCard.timeAgo.hours', { count: hours });
  
  const days = Math.floor(hours / 24);
  if (days < 7) return t('videoCard.timeAgo.days', { count: days });
  
  const weeks = Math.floor(days / 7);
  if (weeks < 4) return t('videoCard.timeAgo.weeks', { count: weeks });
  
  const months = Math.floor(days / 30);
  if (months < 12) return t('videoCard.timeAgo.months', { count: months });
  
  const years = Math.floor(days / 365);
  return t('videoCard.timeAgo.years', { count: years });
}
...
export const VideoCard: React.FC<VideoCardProps> = ({ video, onToggleMenu, onVideoClick, modalMode = false }) => {
  ...
  const formattedPublished = useMemo(
    () => video.published ? formatRelativeTime(video.published, t) : "",
    [video.published, t]
  );
